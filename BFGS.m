...
%   O algoritmo combina o método de Quase-Newton BFGS e uma técnica de 
...
%
%
%  function [sol,ierr,output]=BFGS(x,F,tol,l,u,parms)
%  -----------------------------------------------------
%  
...
%         =  3   o raio de região de confiança Delta ficou muito pequeno
%                (Delta<sqrt(eps));
...
%
%
%  function [sol,ierr,output]=BFGS(x,F,tol,l,u,parms,Fjac)
%  ---------------------------------------------------------- 
%
...
%
%
%  function [sol,ierr,output,history]=BFGS(x,F,tol,l,u,parms,..) 
%  ---------------------------------------------------------- 
%
...
%
%
%  function [sol,ierr,output,history,grad]=BFGS(x,F,tol,l,u,parms,..)
%  ---------------------------------------------------------------------
%
...
ajac   = 0;           %contador do número de cálculos de J
nstep_back = 0;       %contador de step-backs
ncirc  = 0;           %contador de cálculos de J no círculo interno
natual = 0;           %contador de vezes em que B não é atualizado
...
% Saída inicial
    ...
    if outflag>0
        ...
        fprintf('*                                                                       *\n')
        fprintf('*  Um solucionador para SISTEMAS QUADRADOS DE EQUAÇÕES NÃO              *\n')
        fprintf('*                       LINEARES                                        *\n')
        fprintf('*                                                                       *\n')
        fprintf('*************************************************************************\n')
        fprintf('\n')
        fprintf('    It     ||F||_2      rid_step     alpha       ratio        direc \n') 
        fprintf('-------------------------------------------------------------------------\n')
...
% Atualização da matriz B_{k+1} via BFGS
if (abs(den2)>1e-8) & (abs(den1)>1e-8)
    jac = jac - ((jac*p)*(p'*jac))/(den1) + (y*y')/(den2);
else
    %Atualize B_{k+1} pela fórmula SR1
    prod = y-jac*p;
    den  = (prod)'*p;
...
% Registre no vetor tstep[itc] o número de cada passo aceito
    if step=='d'
        tstep(1) = tstep(1)+1;
    elseif step=='qn'
        tstep(2) = tstep(2)+1;
...
%  Saídas finais 
  ...
  % Avaliação da Jacobiana para calcular grad e, depois , D^(-1)*grad
     if nargin==6
       jac = diffjac(x,F,fx,l,u);
     else
       jac = feval(Fjac,x);
...
%
% OBS.:
%    -> alp   : Corresponde a \Lambda no artigo
