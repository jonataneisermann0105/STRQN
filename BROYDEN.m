function[sol,ierr,output,history,grad,diagnostic,tempo]= ...
                                    BROYDEN(x,F,tol,l,u,parms,Fjac)
%  
...
%   O algoritmo combina o método de Quase-Newton Broyden e uma técnica de 
...
%
%
%  function [sol,ierr,output]=BROYDEN(x,F,tol,l,u,parms)
%  -----------------------------------------------------
%  
...
%
%
%  function [sol,ierr,output]=BROYDEN(x,F,tol,l,u,parms,Fjac)
%  ---------------------------------------------------------- 
%
...
%
%
%  function [sol,ierr,output,history]=BROYDEN(x,F,tol,l,u,parms,..) 
%  ---------------------------------------------------------- 
%
...
%
%
%  function [sol,ierr,output,history,grad]=BROYDEN(x,F,tol,l,u,parms,..)
%  ---------------------------------------------------------------------
%
...
ajac   = 0;           %contador de cálculos de J
nstep_back = 0;       %contador de step-back
...
%Variáveis para a atualização da Jacobiana
y = fxpp - fx;
...
% Atualização da matriz B_{k+1} via Broyden
if den1 > (1e-8)
    jac = jac + ((y-jac*p)*p')/(den1);
else
...
%  Saídas finais 
 ...
 % Avaliação da Jacobiana para o cálculo de grad e, depois, de D^(-1)*grad
     if nargin==6
       jac = diffjac(x,F,fx,l,u);
     else
       jac = feval(Fjac,x);
